...
export class MatchRepository {
  async exists(lostId: string, foundId: string): Promise<boolean> {
    const query = 'SELECT 1 FROM matches WHERE lost_declaration_id = $1 AND found_declaration_id = $2';
    const { rows } = await pool.query(query, [lostId, foundId]);
    return rows.length > 0;
  }

  async create(lostId: string, foundId: string, score: number, status: string = 'PENDING') {
    const query = `
