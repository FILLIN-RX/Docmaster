...
export default function Parrainage() {
  ...
  const shareVia = (platform) => {
    const text = `Rejoins DocMaster avec mon code ${code} et profite d'avantages exclusifs ! ${refLink}`;
  ...
  if (loading) {
    return (
      <div className="flex flex-col h-full">
        <Topbar title="Parrainage" breadcrumbs={[{ label: "Accueil", href: "/dashboard" }, { label: "Parrainage" }]} />
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="custom-scroll p-4 md:p-6 flex flex-col gap-6 pb-24 md:pb-6" style={{ height: "calc(100vh - 64px)", overflowY: "auto" }}>
        ...
        <div className="flex items-start justify-between gap-3 flex-wrap mb-5">
          <div>
            <p className="text-[13px] text-textMuted font-medium italic">Suivez votre programme de parrainage</p>
          </div>
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-center gap-5">
          <div className="flex-1">
            <div className="inline-flex items-center gap-2 bg-primary/15 border border-primary/25 rounded-full px-3 py-1 mb-3">
              ...
              <span className="text-[11px] font-bold text-primary uppercase tracking-wide">Programme de parrainage</span>
            </div>
            <h1 className="font-bricolage text-2xl sm:text-3xl font-extrabold text-white leading-tight mb-2">
              Parrainez et gagnez ensemble
              <i className="fa-solid fa-sparkles text-primary text-xl align-middle ml-1" aria-hidden="true" />
            </h1>
            <p className="text-white/80 text-[14px] leading-relaxed max-w-[560px]">Invitez vos proches sur DocMaster. Pour chaque parrainage validé, vous et votre filleul recevez des avantages exclusifs.</p>
          </div>
          <div className="bg-white/[0.08] border border-white/10 rounded-[18px] p-5 flex-shrink-0 text-center min-w-[160px]">
            ...
            <div className="text-white/60 text-[12px] font-medium mt-1">Filleuls actifs</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-5">
        <div className="sm:col-span-2 lg:col-span-1 bg-white border border-borda rounded-[18px] p-5">
          <p className="text-[11px] font-bold text-textMuted uppercase tracking-widest mb-3">Mon code de parrainage</p>
          <div className="flex items-center gap-2 bg-bgMain border border-borda rounded-[12px] px-4 py-3 mb-3">
            ...
            <button onClick={copyCode} className="flex items-center gap-1.5 px-3 py-1.5 bg-primary text-white text-[12px] font-bold rounded-[8px] transition-all hover:bg-primary-dark">
              <i className={`${copied ? "fa-solid fa-check" : "fa-regular fa-copy"} text-[11px]`} /> {copied ? "Copié" : "Copier"}
            </button>
          </div>
        ...
        <div className="bg-white border border-borda rounded-[18px] p-5 flex flex-col gap-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] font-bold py-1 px-2.5 rounded-full bg-green-light text-green-mid">Ce mois</span>
          </div>
          <div>
            ...
            <div className="text-[12.5px] text-textMuted font-medium">Gains cumulés</div>
          </div>
          <div className="h-1.5 bg-bgMain rounded-full overflow-hidden">
            <div className="h-full bg-primary rounded-full" style={{ width: `${progress}%` }} />
          </div>
          <div className="text-[11.5px] text-textMuted">Prochain palier : <span className="font-bold text-textMain">{nextPayout.toLocaleString("fr-FR")} XAF</span></div>
        </div>
        ...
        <div className="bg-white border border-borda rounded-[18px] p-5 flex flex-col gap-3">
          ...
          <div>
            ...
            <div className="text-[12.5px] text-textMuted font-medium">Signalements bonus</div>
          </div>
          <div className="flex items-center gap-1.5">
            <i className="fa-solid fa-circle-info text-textMuted text-[11px]" />
            <span className="text-[11.5px] text-textMuted">2 slots par parrainage validé</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-borda rounded-[20px] p-5 sm:p-6">
        <h2 className="font-bricolage text-lg font-bold text-textMain mb-5 flex items-center gap-2">
          <i className="fa-solid fa-circle-question text-primary text-base" /> Comment ça marche ?
        </h2>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            { step: "1", icon: "fa-solid fa-link", title: "Partagez votre code", desc: "Copiez votre code de parrainage et partagez-le avec vos proches via WhatsApp, SMS ou vos réseaux sociaux." },
            { step: "2", icon: "fa-solid fa-user-plus", title: "Ils s'inscrivent", desc: "Vos proches créent leur compte DocMaster en utilisant votre code de parrainage lors de l'inscription." },
            { step: "3", icon: "fa-solid fa-coins", title: "Vous gagnez", desc: "Pour chaque filleul qui valide son compte, vous recevez des bonus et des avantages exclusifs." },
          ].map((item) => (
            <div key={item.step} className="flex sm:flex-col items-start sm:items-center gap-4 sm:gap-3 sm:text-center">
              ...
              <div>
                <p className="text-[14px] font-bold text-textMain">{item.title}</p>
                <p className="text-[12px] text-textMuted mt-0.5">{item.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {friends.length > 0 && (
        <div className="bg-white border border-borda rounded-[20px] p-5 sm:p-6 mt-5">
          <h2 className="font-bricolage text-lg font-bold text-textMain mb-4">Mes filleuls ({friends.length})</h2>
